<?php
    ...
    if($action === 'save'){
        ...
        if(move_uploaded_file($temp_img, $path2upload)){
            ...
            $tax=$_POST['tax'];
            $total= $_POST['total'];
            $insert_product = $conn->query("INSERT INTO tbl_products(product, category_id, price, image, description,total,
             grand_total,quantity,tax) VALUES('$product','$cat_id','$price','$name2store','$desc','$total','$gtotal','$quantity','$tax')");
        ...
        if(isset($_FILES['image'])){
            ...
            $update_product = $conn->query("UPDATE tbl_products SET product='$product', category_id='$cat_id', 
                price='$price', image='$name2store', description='$desc', total='$total'
                , grand_total='$gtotal', quantity='$quantity', tax='$tax' WHERE id='$product_id'");
